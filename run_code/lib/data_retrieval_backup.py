...
class getData:
    ...
    def download_retrospective(self,days):
        ...
        # https://rda.ucar.edu/data/ds628.0/fcst_surf125/1959/fcst_surf125.118_brtmp.1958010100_1958123121          # https://rda.ucar.edu/data/ds628.0/anl_land125/2020/anl_land125.225_soilw.2020010100_2020013118
        ...
        for key in keys:   
            ...
            for day in days:
                ...
                if key == 'phy2m':
                    ...
                    ds = xr.merge([xr.open_dataset(f,engine='cfgrib',backend_kwargs={'filter_by_keys':{'cfVarName':'brtmp'}}) for f in files]).sel(time=str(day.year)+'-'+str(day.month)+'-'+str(day.day))  
                    ds = ds.sel(step=timedelta(hours=3)) # select 3 hour forecast to match real-time 
                else:

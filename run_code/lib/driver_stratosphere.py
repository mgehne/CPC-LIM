...
class Driver:
    ...
    def get_variables(self,read=True,save_netcdf_path=None,segmentby=None):
        ...
        if read:
            for name in self.use_vars.keys():
                print('reading pickles from ', f"{self.VAR_FILE_PREFIX}{name}.p")
                self.use_vars[name] = pickle.load( open( f"{self.VAR_FILE_PREFIX}{name}.p", "rb" ) )
                print(self.use_vars[name])
        else:
        ...
        if read:
            for limkey,eof_lim in self.eof_trunc_reg.items():
                ...
                for key in eof_lim.keys():
                    print(f'reading {key} for LIM {limkey}, Line 125 in driver')
                    self.eofobjs[limkey][key] = pickle.load( open( self.EOF_FILE_PREFIX+'+'.join(listify(key))+f'_{limkey}.p', "rb" ) )
                    # print(self.eofobjs[limkey][key])# This is an eofDataset obj
                    print('Line 129 in driver, reading pickle file: ',dir(self.eofobjs[limkey][key]))# This has varobjs
                    # print(self.eofobjs[limkey][key].eof_dict)
                    # self.eofobjs[limkey][key].eof_dict has keys of eof, total_var, var_expl_by_eof, pc
                    # print(self.eofobjs[limkey][key].eof_dict['eof'][0])# This has the eof patterns and values
                    print(type(self.eofobjs[limkey][key].eof_dict['eof'][0]))# np array
                    print((self.eofobjs[limkey][key].eof_dict['eof'][0]).shape)
                    # for eofobj in self.eofobjs[limkey][key].varobjs: 
                    #     print('Line 130 in driver ',eofobj.varlabel)# This is an varDataset obj
                    #     print(dir(eofobj))
            # pc = get_eofs(prepped,eof_in=eofobjs[('H100', 'H10')].eof_dict['eof'][:eof_lim[('H100', 'H10')]])
                        
            # exit()        
        if not read:
            # Save EOF objects for each season
            for limkey in self.eof_trunc_reg.keys():# 1, 2, 3,...12
                #Get EOF truncations for the given LIM
                eof_lim = self.eof_trunc_reg[limkey]
                print('Line 182 driver.py eof_lim',self.eof_trunc_reg[limkey])
                ...
                if isinstance(limkey,int):
                    ...
                    tmpobjs = {}
                    for name in self.use_vars.keys():
                        print(name,'Line 141 driver.py, reading ',f"{self.VAR_FILE_PREFIX}{name}.p")
                        tmp = pickle.load( open( f"{self.VAR_FILE_PREFIX}{name}.p", "rb" ) )                        
                        tmpobjs[name] = copy.copy(tmp['data'])
                        tmpobjs[name].subset((t1,t2))
                        print(tmpobjs[name],'at line 145, driver.py')
                        print(tmpobjs)# is this varDataset or eofDataset obj?

                #Calculate EOFs of the subset variable objects using eof_lim_keys, e.g., dict_keys(['H500', ('H100', 'H10')])
                print(eof_lim.keys())#dict_keys(['H500', ('H100', 'H10')])
                for key in eof_lim.keys():
                    print('Line 146, driver.py',limkey,key)
                    # print(listify(key))#['H500']
                    # each key is listify, so ('H100', 'H10')->['H100','H10']
                    eofobj = eofDataset([tmpobjs[k] for k in listify(key)])
                    print(dir(eofobj))
                    # This is first derived from varDataset object from reading in pickle files:
                    # tmp = pickle.load( open( f"{self.VAR_FILE_PREFIX}{name}.p", "rb" ) )
                    # Then it becomes an eofobj object, so it has both varData and eofDataset methods and attributes
                    # tmpobjs is a dictionary, k is the names from , which is the key to the dictionary
                    #  Input a list of tmpobjs in eof_lim.key, so this object will have both 'H100' and 'H10'. 
                    #  This is NOT a loop that includes 2 vars
        ...
        if save_netcdf_path is not None:
            for limkey in self.eof_trunc_reg.keys():
                for key in eof_lim.keys():
                    ...
                    print('Line 176 in driver.py')
                    # print(eofobj)
                    print(dir(eofobj))
                    print(eofobj)# This is an eofDataset obj.
                    if not os.path.isdir(save_netcdf_path+f'EOF_{limkey}'):
                        os.mkdir(save_netcdf_path+f'EOF_{limkey}')
                    eofobj.save_to_netcdf(save_netcdf_path+f'EOF_{limkey}')# But eof.obj has varDataset obj. As save_to_netcdf uses var
    ...
    def prep_realtime_data(self,limkey,verbose=True):
        ...
        if 'time' not in self.RT_VARS.keys():
            print('we are in Line 588 of driver.py, no time in self.RT_VARS.keys()')
            for name in self.RT_VARS.keys():
                ...
                if 'level' in self.RT_VARS[name].keys():
                    print(f"{name} has level")
                    levs = ds[self.RT_VARS[name]['levname']][:]
                    # level_for_var = [(np.where(levs == i)[0]) for i in self.RT_VARS[name]['level']]
                    # print(levs)
                    if len(listify(self.RT_VARS[name]['level'])) > 1:
                        varname_input = name
                        additional_vars = varname_input.split("+")
                        RT_VARS_additional = {}
                        for ll, vartmp in enumerate(additional_vars):
                            level_for_var = np.where(levs==self.RT_VARS[name]['level'][ll])[0]
                            newdata_lev = newdata[:,level_for_var].squeeze()
                            print(level_for_var)
                            RT_VARS_additional[vartmp]={}
                            print(newdata_lev.shape)
                            newdata_lev = np.apply_along_axis(lambda x: np.convolve(x,np.ones(perday)/perday, mode='valid')[::perday],\
                                                                axis=0, arr=newdata_lev)
                            running_mean = get_running_mean(newdata_lev,7)[7:]
                            RT_VARS_additional[vartmp]['var'] = running_mean
                            RT_VARS_additional[vartmp]['lat'] = ds['latitude'][:]
                            RT_VARS_additional[vartmp]['lon'] = ds['longitude'][:]
                            RT_VARS_additional[vartmp]['time'] = times[7:]
                        print('Line 659 driver.py',RT_VARS_additional.keys())
                    else:
                        print(f'processing {name} at one level option of prep_realtime_data')
                ...
                else:# not H500 nor H10/H100
                    print('we do not have level here in prep_realtime_data, Line 673 driver.py')
                    newdata = np.apply_along_axis(lambda x: np.convolve(x,np.ones(perday)/perday, mode='valid')[::perday],\
                                                                axis=0, arr=newdata)
                    running_mean = get_running_mean(newdata,7)[7:]
                    ...
                    self.RT_VARS[name]['lon'] = ds['longitude'][:]
                    self.RT_VARS[name]['time'] = times[7:]
            print(RT_VARS_additional.keys())
            # get rid of key 'H100+H10' for now and replace it with 'H100' and 'H10' 
            # This is becasue we want to use the interpolation and anomaly in the next section
            # We need to make sure H100 and H100 are read in in use_vars and levels are specified in RT_VARS 
            if isinstance(varname_input,str): 
                self.RT_VARS.pop(varname_input)
            for count, key_additional in enumerate(RT_VARS_additional.keys()):
                self.RT_VARS[key_additional] = RT_VARS_additional[key_additional]
            print(RT_VARS_additional.keys())
            print(self.RT_VARS.keys())
            # find all common times
            p = [v['time'] for v in self.RT_VARS.values()]
            common_times = set(p[0]).intersection(*p)
            # print(sorted(common_times))
            print('end of common_times Line 625 driver.py')
            ...
            for name in self.RT_VARS.keys():
                ...
                # print(len(self.use_vars[name]['data'].lat))#1620; 2880
                # print(len(self.use_vars[name]['data'].lon))#1620; 2880
                print(self.RT_VARS[name]['lat'].shape)#73; 73
                print(self.RT_VARS[name]['lon'].shape)#144; 144
                # self.RT_VARS[name]['lat'] and self.RT_VARS[name]['lon'] are 2.5 degree, 73x144
                # for var_day in data:
                    # print(var_day.shape)
                ...
                # RT_INTERP[name] = np.array([interp2LIM(self.RT_VARS[name]['lat'],self.RT_VARS[name]['lon'],\
                #                            var_day,self.use_vars[name]['data']) for var_day in data[0:2,:,:]])                
            ...
            for name in self.RT_VARS.keys():
                ...
                prepped = get_anomaly(RT_INTERP[name],self.RT_VARS[name]['time'],\
                                                 self.use_vars[name]['data'].climo)
                prepped = get_area_weighted(prepped,self.use_vars[name]['data'].lat)
                prepped = prepped / self.use_vars[name]['data'].climo_stdev
                self.RT_ANOM[name] = prepped
                
            self.RT_VARS['time'] = self.RT_VARS[name]['time']
        # We are putting 'H100+H10' back to the dictionary and get rid of the individual 'H100' and 'H10'
        # First contacenate H100 and H10 in data
        
        try:
            data_concatenate = []
            print('we are in the trying phase line 740 driver.py')
            for key_additional in RT_VARS_additional.keys():
                print(key_additional)
                print(self.RT_ANOM[key_additional])
                data_concatenate.append(self.RT_ANOM[key_additional])
                print(self.RT_ANOM[key_additional].shape)
                self.RT_ANOM.pop(key_additional)
                # This is where H10 and H100 are combined along the lat dim
            data_concatenate = np.concatenate(data_concatenate,axis=1)
            self.RT_ANOM[varname_input] = data_concatenate
            print(self.RT_ANOM.keys()) 
        except NameError:
            print('we are NOT in the trying phase line 748 driver.py')
        ...
        eof_lim = self.eof_trunc[limkey]#eof_lim is a dictionary with dict_keys(['H500', ('H100', 'H10')])
        eofobjs = self.eofobjs[limkey]#eofobjs is a dictionary with dict_keys(['H500', ('H100', 'H10')])

        # print('Line 742 in driver',type(eof_objs))
        # print(eof_lim.keys())# How many eof_var are there in eofobjs for ('H100','H10')?
        # print(eofobjs.keys())# How many eof_var are there in eofobjs for ('H100','H10')?
        
        for name in eof_lim.keys():  print(f"eof_lim.keys(): {name}") 
        #1st: H500; 2nd: ('H100','H10')

        for name in eofobjs.keys():  print(f'Line 722; eofobjs.keys(): {name}')
        #1st: H500; 2nd: ('H100','H10')
        
        # print(eofobjs["500"])
        self.RT_PCS = {}
        for name in self.RT_ANOM.keys():
        # for name in eofobjs.keys():# CYM
            # prepped = get_area_weighted(self.RT_ANOM[name],self.use_vars[name]['data'].lat)
            # prepped = prepped / self.use_vars[name]['data'].climo_stdev
            # We doing area weighting and anomalies again, skip for now
            print(name)
            prepped = self.RT_ANOM[name]
            if name == 'H100+H10':
                name = ('H100', 'H10')
            print(name)
            print(eofobjs[name].eof_dict['eof'][0].shape)
            print(prepped.shape)
            # print((self.eofobjs[limkey][key].eof_dict['eof'][0]).shape) is 2880

            # print((listify(eofobjs[name].keys())))
            # print(type(listify(eofobjs[name].keys())))
            # a = .join(listify(eofobjs[name])).eof_dict['eof'][:eof_lim[name]]
            # print('len a = ',len(a))
            # This is getting the pcs for    self.RT_ANOM
            pc = get_eofs(prepped,eof_in=eofobjs[name].eof_dict['eof'][:eof_lim[name]])
            # pc = get_eofs(prepped,eof_in=eofobjs[('H100', 'H10')].eof_dict['eof'][:eof_lim[('H100', 'H10')]])
            self.RT_PCS[name] = pc

    # def prep_raw_data(self,limkey,verbose=True):

    #     r"""
    #     Compile realtime data, interpolate to same grid as LIM, and convert into PCs.
    #     by CYM
    #     """
    #     Data_path = '/data/ycheng/JRA/Data/'
    #     year=2000
    #     self.filedict = {\
    #     'H100':{'filename':f'{Data_path}/{year}/hgt_{year}.nc','varname': 'gh', 'level':100,'levname':'isobaricInhPa'},\
    #     'H500':{'filename':f'{Data_path}/{year}/hgt_{year}.nc','varname': 'gh', 'level':500,'levname':'isobaricInhPa'},\
    #     'SLP' :{'filename':f'{Data_path}/{year}/surf_{year}.nc' ,'varname':'msl'},
    #     'T2m' :{'filename':f'{Data_path}/{year}/surf_{year}.nc' ,'varname':'t2m'},
    #     'colIrr':{'filename':f'{Data_path}/{year}/phy2m_{year}.nc','varname':'colIrr'},
    #        }
        
    #     for name in self.filedict.keys():
    #         if verbose:print(f'reading {name}')
    #         ds = nc.Dataset(self.filedict[f'{name}']['filename'])
    #         print(ds)
    ...
    #         newdata = ds[self.filedict[name]['varname']][:]

    #         if 'level' in self.filedict[name].keys():
    #             levs = ds[self.filedict[name]['levname']][:]
    #             level_for_var = np.where(levs==self.filedict[name]['level'])[0]
    #             newdata = newdata[:,level_for_var].squeeze()
    #             print(level_for_var)
    ...
    #         self.filedict[name]['var'] = newdata
    #         self.filedict[name]['lat'] = ds['latitude'][:]
    #         self.filedict[name]['lon'] = ds['longitude'][:]
    #         self.filedict[name]['time'] = times
    ...
    #     for name in self.filedict.keys():
    #         if verbose:print(f'interp {name}')
    #         data = self.filedict[name]['var']
    #         data[np.isnan(data)] = 0
    #         RT_INTERP[name] = np.array([interp2LIM(self.filedict[name]['lat'],self.filedict[name]['lon'],\
    #                                     var_day,self.use_vars[name]['data']) for var_day in data])
    #         # need to make directory first...
    #         RT_INTERP[name].save_to_netcdf(f'{Data_path}/{self.filedict[name]}/{self.filedict[name]}_{year}.nc')
    #         # self.filedict[name]['var'] = varDataset(name,*self.filedict[name]['info'][:-1],**self.use_vars[name]['info'][-1])
    #         # self,varlabel,datapath,varname,**kwargs)
    #         # self.RT_ANOM = {}
    #         # for name in self.RT_VARS.keys():
    #         #     if verbose:print(f'anom {name}')
    #         #     self.RT_ANOM[name] = get_anomaly(RT_INTERP[name],self.RT_VARS[name]['time'],\

...
T_START = dt(2023,1,1) #dt(YEAR,MONTH,1)
T_END = dt(2023,8,13) #dt(YEAR,MONTH,LASTDAY)
...
for T_INIT in FORECASTDAYS:
    ...
    def make_maps(LT):
        LIMdriver.plot_map(varname=Tvar,t_init=T_INIT,lead_times=LT,fullVariance=fullVariance,pc_convert=pc_convert,add_offset=tclim_file,add_offset_sliding_climo=True, gridded=True,\
  ...
  #                  prop={'levels':np.linspace(-5,5,21),'interpolate':.25,'cbar_label':'$^oC$','dpi':DPI,'addtext':credit},save_to_path = FCSTDIR)
    ...
    try:
        ...
        LIMdriver.save_netcdf_files(varname=Tvar,t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=FCSTDIR,add_offset=tclim_file,add_offset_sliding_climo=True,append_name=var_name_append)

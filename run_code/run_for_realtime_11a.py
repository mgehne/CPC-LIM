...
t0=dt(2023,8,7).replace(hour=0,minute=0,second=0,microsecond=0)
...
# FORECASTDAYS = [t0+timedelta(days=i-14) for i in range(4)]
...
for T_INIT in FORECASTDAYS:
    ...
    if T_INIT<dt(2021,5,29):
        climoffsetfile = '/Projects/jalbers_process/CPC_LIM/yuan_ming/Data/offline_climatology/2p0.1981-2010/data_clim/2p0.1981-2010_T2m.nc'
    else:
        climoffsetfile = '/Projects/jalbers_process/CPC_LIM/yuan_ming/Data/offline_climatology/2p0.1981-2010/data_clim/2p0.1981-2010_T2m.nc'
    ...
    def make_maps(LT):
        LIMdriver.plot_map(varname='T2m',t_init=T_INIT,lead_times=LT,fullVariance=fullVariance,pc_convert=pc_convert,add_offset=climoffsetfile,add_offset_sliding_climo=True, gridded=True,\
        ...
        LIMdriver.plot_map(varname='T2m',t_init=T_INIT,lead_times=LT,fullVariance=fullVariance,pc_convert=pc_convert,add_offset=None, gridded=True,\
                    prop={'levels':np.linspace(-5,5,21),'interpolate':.25,'cbar_label':'$^oC$','dpi':DPI,'addtext':credit},save_to_path = f'{FCSTDIR}/no_offset')
    ...
    try:
        ...
        LIMdriver.save_netcdf_files(varname='T2m'   ,t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=FCSTDIR,add_offset=climoffsetfile,add_offset_sliding_climo=True,append_name=var_name_append)
        LIMdriver.save_netcdf_files(varname='T2m'   ,t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=f'{FCSTDIR}/no_offset',add_offset=None)
        ...
        LIMdriver.save_netcdf_files(varname='SLP'   ,t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='H500'  ,t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='colIrr',t_init=T_INIT,lead_times=(0,14,21,28),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='SF100' ,t_init=T_INIT,lead_times=(21+dayoffset,28+dayoffset),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='SF750' ,t_init=T_INIT,lead_times=(21+dayoffset,28+dayoffset),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='SST'   ,t_init=T_INIT,lead_times=(21+dayoffset,28+dayoffset),save_to_path=FCSTDIR)
        LIMdriver.save_netcdf_files(varname='SOIL'  ,t_init=T_INIT,lead_times=(21+dayoffset,28+dayoffset),save_to_path=FCSTDIR)
        
        print(f'SAVING CPC PERIOD FORECAST FOR {T_INIT:%Y%m%d}')
        var_name_append = '_Week_34_official_CPC_period'
        LIMdriver.save_netcdf_files(varname=Tvar,t_init=T_INIT,lead_times=(0+dayoffset,14+dayoffset,21+dayoffset,28+dayoffset),save_to_path=FCSTDIR,add_offset=climoffsetfile,add_offset_sliding_climo=True,append_name=var_name_append)
        LIMdriver.save_netcdf_files(varname=Tvar,t_init=T_INIT,lead_times=(0+dayoffset,14+dayoffset,21+dayoffset,28+dayoffset),save_to_path=f'{FCSTDIR}/no_offset',add_offset=None,append_name=var_name_append)
